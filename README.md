...
没有改区域的接口。因此后端按 `window_track_interval_ms`（默认 5000 ms）重新解析，
只在矩形真的变化时重建采集源；窗口暂时找不到时（最小化、Alt-Tab）保留上一次的矩形，
不回退全屏——把整个桌面喂给检测器比暂时抓一块过期区域危险得多。设为 0 可恢复
"只在启动时解析一次"的旧行为。该参数对 `wgc` 无效。
...
检测器只回答"有几个人、在哪个方位"。要判断前方几何是否可能正在接近障碍，独立的
`VisionWorker` 还会从连续帧计算短期 `traversability_prediction`；它只输出分扇区的
发散风险、接触时间估计、置信度或 `unknown`，不创建实体、不提供米制距离，也不写入
持久化世界。转向、特征不足或画面跳变时必须按 `unknown` 处理。要确认对方是谁、菜单开着没、界面上写了什么，
...
AnyaDance 调度器仍以 120 Hz 发送最新姿态与控制器状态。任何朝视觉实体移动的目标都必须
携带由 LLM 从最新世界快照中选出的精确 `target_id`；没有 ID 的探索请求只观察，不移动。
对于 wander，连续帧几何预测只作为主 LLM 已选方向的安全门：高置信 `predicted_blocked`
会停车并把原因交回主 LLM，不会在本地偷偷改选路线；`unknown` 绝不等于空旷。
本地导航不会按文字、标签或置信度自动锁定实体。方位角和接近度在本地做轻量时序平滑，单帧低置信或漏检最多
...
当画面中有多个 person 候选时，主多模态 LLM 先调用 `world_observe`，再调用
`vrc_vision_frame(overlay=true)`。叠框会显示仅对该帧有效的 `T1`、`T2`，工具结果的
`overlay.candidates` 同步给出每个 T 编号对应的完整 `target_id`。LLM 用画面排除看起来像
海报、屏幕或镜像的候选，再把映射中的完整 ID 提交给自主导航；`T1` 本身绝不能作为
`target_id`。JPEG、该帧检测实体和 revision 在检测完成后原子地覆盖同一个内存单槽；
不保留帧历史、不创建临时图片，也不增加世界存储写入。只有 `overlay.paired=true` 才会
画框，因此不会再拿缓存旧图叠最新世界。叠框失败或候选为空时不会发起移动。

这个稳定 ID 不是 VRChat 的 `usr_`/`avtr_`，后端重启后会变化，也无法可靠区分使用
完全相同 Avatar 的玩家或镜中像；两个目标同时可见时绝不合并，历史模板歧义只有在
背景、短时几何或长期稳定度证据足够明显时才复用，否则仍分配新 ID。
