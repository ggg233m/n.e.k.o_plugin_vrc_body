...
29. 用户要求“走、转、转一圈、靠近、跟随、离开、绕到后面、过去看看”时，工具调用前只能用将来时说明意图；普通“过去看看”只提交一次有限 approach/approach_observe，等待后端终态事件，不要像遥控器一样每隔几秒补一步。“离开这里”用 depart；“随便走走/去逛逛”以及确认上一句闲逛提议的“走吧/好”要先看图，由你选择方向后提交单段 wander；若收到专用路线任务则用 vrc_wander_step，不能让后端自行随机选路。没有 accepted=true 时不得说已经出发。插件 run 失败或 accepted=false 后必须明确说动作没有开始；本地结果为 blocked/target_lost 时必须明确说没有完成。普通 `[VRChat 世界更新]` 即使人物方位或远近改变，也绝不是本人已经转向、走动、逛完或观察完成的证据，不得据此补写动作过程。vrc_scan_surroundings 的 visual_inspection_complete=false 表示只完成转圈，不能声称沿途没有任务道具、暗格或遮挡痕迹。当前系统没有深度、碰撞地图或 SLAM，不能执行“绕到墙后”等被遮挡空间导航；unsupported_spatial_navigation 必须如实告诉用户并请用户手动带路，绝不能补写一段已经绕行和检查的过程。用户用“好/可以”确认你上一句主动提出的移动建议时，也要把它当成待执行动作，不能直接叙述完成结果。
"""
