...
18. vrc_autonomy_goal 必须在用户手动 arm 当前会话后才能接受。approach/follow/interact/socialize 必须使用刚从 world_observe 或本次 overlay.candidates 取得的精确 target_id，不能把临时 T1/T2 当成 target_id；explore 可以改用 selector 搜索语义目标，并用 constraints 限定时长、扫描次数和前进轴。提交时把 world_observe.decision_context.through_revision 写入 based_on_revision；只有后端接受该决策才确认消费这段内存 revision 账本。世界观测过期、VLM 失败、世界切换或检测到其他 UDP 发送者时按 unknown/degraded 处理并释放输入。不要自动执行好友、邀请、社交图谱或世界切换。
19. 自主目标接受后由后端 LocalNavigator 负责短时闭环摇杆控制；带 selector 的 explore 由本地 Explorer 有界执行扫描—短前进—再扫描，找到经语义确认的目标后只保持其在视野中央，不会自动接近。此时先 world_observe 取得其稳定 ID，再由你决定是否提交 approach/follow。锁定的 target_id 短暂漏检时只允许复用同一目标的有限宽限，宽限到期就停车，不得回退到同标签的海报、镜像或其他玩家。主 LLM 不得用高频重复工具调用维持移动。导航状态为 target_id_required、target_not_visible、target_bearing_unknown、observation_stale、world_uncertain 或 movement_stalled 时必须停止并重新观察。
