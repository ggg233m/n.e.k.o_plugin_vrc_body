    ...
    def __init__(
        ...
        **kwargs,  # to catch addition args in configs
        ...
        if load and folder is not None:
            pfolder = Path(folder)
            # 资源可能保存为 FP64；运行时与动作网络统一为 FP32，兼容无 FP64 的 Intel Arc。
            neutral_joints = torch.load(pfolder / "joints.p", map_location="cpu").squeeze().float()
            self.register_buffer("neutral_joints", neutral_joints, persistent=False)

            if (pfolder / "bvh_joints.p").exists():
                bvh_neutral_joints = torch.load(pfolder / "bvh_joints.p", map_location="cpu").squeeze().float()
            ...
            if global_offset_path.exists():
                global_rot_offsets = torch.load(global_offset_path, map_location="cpu").squeeze().float()
