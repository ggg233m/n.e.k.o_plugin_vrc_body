...
def load_text_encoder(
    ...
    Args:
        ...
        device: 目标设备；XPU 机器默认使用 CPU 承载文本编码器。
    ...
    device = resolve_text_device(device)
    ...
    if text_encoder is None:
        # 构造时直接传入目标设备，避免先占用其他显卡再迁移。
        conf = dict(conf)
        if conf["_target_"] == "ardy.model.LLM2VecEncoder":
            conf["device"] = device
...
def load_model(
    ...
    Args:
        ...
        text_encoder: Pre-built text encoder to reuse, or False to load the
            ...
            ``load_text_encoder``.
        text_encoder_device: 独立文本编码设备；XPU 动作推理时默认 CPU。
    ...
    if text_encoder is False:
        ...
        text_encoder = load_text_encoder(
            ...
            device=resolve_text_device(text_encoder_device, device),
        )
