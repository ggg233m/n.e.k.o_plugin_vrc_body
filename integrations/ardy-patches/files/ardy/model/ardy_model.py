...
class Ardy(nn.Module):
    ...
    def compile_denoiser(
        ...
        if backend == "torch_tensorrt" and torch.device(self.device).type != "cuda":
            raise ValueError("TensorRT 仅支持 CUDA；XPU 请使用 eager 或 inductor。")
    ...
    def denoising_step(
        ...
        prepared_sampling: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = None,
        generation_token_range: Optional[Tuple[int, int]] = None,
    ) -> torch.Tensor:
        ...
        if generation_token_range is None:
            ...
            # 窗口内生成区间对所有样本一致，切片避免布尔索引的动态长度同步。
    ...
    def _generate_window(
        ...
        if torch.device(device).type == "xpu" and getattr(self, "xpu_eager_optimizations", True):
            # 每个窗口的采样步数和引导权重不变，只计算一次，避免每步重复提交。
            ...
            for i in progress_bar(indices):
                ...
                with torch.no_grad():
                    x = self.denoising_step(
                        ...
                        generation_token_range=generation_token_range,
                    )
