...
class AutoLatentClassifierFreeGuidedModelSeparated(nn.Module):
    ...
    def forward(
        ...
        from ardy.model.eager_window import current_window, cfg_forward
        window = None if torch.compiler.is_compiling() else current_window()
        if window is not None and window['cfg'] is self and not self.training:
            return cfg_forward(self, window, locals())
