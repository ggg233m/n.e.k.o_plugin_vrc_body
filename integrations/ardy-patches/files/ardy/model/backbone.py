...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(
        ...
        """
        from ardy.model.eager_window import current_window, backbone_forward
        window = None if torch.compiler.is_compiling() else current_window()
        if (window is not None and not self.training
                and self.positional_encoding_mode == 'learned_prefix_zero_at_first_generation'
                and self in (window['cfg'].model.root_model, window['cfg'].model.body_model)):
            return backbone_forward(self, window, locals())
