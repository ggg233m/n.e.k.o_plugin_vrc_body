...
class RootKeyframe2DSet(ConstraintSet):
    ...
    def remove_keyframe(self, keyframe_id: str, frame_idx: int):
        # 调用方负责串行化批量操作；已清空或重复删除的路点直接忽略。
        keyframe_ids = self.frame2keyid.get(frame_idx)
        if frame_idx not in self.keyframes or not keyframe_ids or keyframe_id not in keyframe_ids:
            return
        keyframe_ids.remove(keyframe_id)
        if not keyframe_ids:
            self.frame2keyid.pop(frame_idx, None)
