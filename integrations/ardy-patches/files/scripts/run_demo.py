...
from ardy.device import resolve_device, resolve_text_device
import argparse
...
from interactive_demo.semantic_bridge import start_bridge
...
class InteractiveTimelineDemo(
    ...
    def __init__(self, compile_model: bool = True, device="auto", text_encoder_device=None):
        self.device = resolve_device(device)
        self.text_encoder_device = resolve_text_device(text_encoder_device, self.device)
        ...
        # 手动和语义提示共用串行提交入口。
        self._text_update_lock = threading.RLock()
        self.semantic_bridge = start_bridge(self)
...
def main() -> None:
    ...
    parser.add_argument("--device", default="auto", help="动作设备：auto / xpu / xpu:0 / cuda / cpu")
    parser.add_argument("--text-encoder-device", default=None, help="文本编码设备；XPU 动作推理时默认 CPU")
