from .display_cache import publish_display_cache_locked
...
from .inference_fallback import autoregressive_step_with_fallback
...
class GenerationMixin:
    ...
    def _generate_step(self, client_id: int):
        ...
        # Generate motion
        samples = autoregressive_step_with_fallback(
            session.model,
        ...
        # 可视化骨骼无需梯度；仅为已安装的编译转换关闭梯度，其他路径保持原状态。
        use_compiled_inverse = getattr(session.motion_rep.inverse, 'xpu_inverse_compiled', False)
        with torch.set_grad_enabled(torch.is_grad_enabled() and not use_compiled_inverse):
        ...
        with session.motion_tensor_lock:
            ...
            if getattr(session, '_display_cache_enabled', True):
                # 只保留有限显示快照，完整 GPU 历史按原方式存储。
                publish_display_cache_locked(session, max(0, min(session.frame_idx, history_end_idx)))
