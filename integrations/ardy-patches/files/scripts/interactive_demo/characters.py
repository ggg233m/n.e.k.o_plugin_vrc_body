...
class CharactersMixin:
    def add_character(self, client_id: int, skeleton: SkeletonBase, index: int):
        ...
        vrm_data = getattr(session, "vrm_data", None) if skeleton.name == "cskel27" else None
        character_type = VrmCharacter if vrm_data is not None else Character
        extra = {"vrm_data": vrm_data} if vrm_data is not None else {}
        new_character = character_type(
            character_name,
            session.client,
            skeleton,
        ...
        if vrm_data is not None:
            new_character.vrm_handle.vrm_spring_bones = getattr(session, "vrm_spring_bones", False)
