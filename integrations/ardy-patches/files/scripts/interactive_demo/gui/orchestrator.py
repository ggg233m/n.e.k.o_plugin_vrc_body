...
class GuiMixin:
    def create_gui(self, client: viser.ClientHandle, constraint_tracks: dict):
        ...
        timeline_data = {
            ...
            "keyframes": {},
            "intervals": {},
            "keyframe_update_lock": threading.RLock(),
    ...
    def on_text_prompt_update(self, client_id: int, trigger_replan: bool = True, initial_prompt: bool = False,
                              prepared_prompt=None, prepared_feature=None):
        # 慢编码与手动更新串行；语义桥可传入已编码且验证时效的提示。
        with self._text_update_lock:
            return self._commit_text_prompt(client_id, trigger_replan, initial_prompt,
                                            prepared_prompt, prepared_feature)

    def _commit_text_prompt(self, client_id, trigger_replan, initial_prompt,
                            prepared_prompt, prepared_feature):
        ...
        text_prompt = prepared_prompt if prepared_prompt is not None else session.gui_elements.gui_prompt_text.value
        if prepared_feature is None:
            text_feat, _ = session.model.text_encoder([text_prompt])
        else:
            text_feat = prepared_feature
        session.gui_elements.gui_prompt_text.value = text_prompt
