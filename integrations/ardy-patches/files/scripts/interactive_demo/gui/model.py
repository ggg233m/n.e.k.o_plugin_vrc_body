    ...
    def _build_model_tab(self, client, client_id, tab_group, g, timeline, default_prompt):
        with tab_group.add_tab("Model", viser.Icon.SETTINGS):
            ...
            g.gui_compile_mode = client.gui.add_dropdown(
                "Acceleration",
                options=["None", "torch.compile"] + (["ONNX-TRT (fp16)", "ONNX-TRT (fp32)"]
                         if torch.device(self.device).type == "cuda" else []),
                initial_value="ONNX-TRT (fp16)" if self.compile_model and torch.device(self.device).type == "cuda" else "None",
            )
            _devices = ["cpu"]
            for _backend in ("cuda", "xpu"):
                if getattr(torch, _backend).is_available():
                    _devices.extend(f"{_backend}:{i}" for i in range(getattr(torch, _backend).device_count()))
            if self.text_encoder_device not in _devices:
                _devices.append(self.text_encoder_device)
            _text_encoder_options = [f"{d} / {p}" for d in _devices for p in ("bfloat16", "float32")]
            _text_encoder_initial = f"{self.text_encoder_device} / bfloat16"
            _quantized = getattr(self.text_encoder, "quantization", "none") != "none"
            if _quantized:
                _text_encoder_options = [_text_encoder_initial]
            _quantization_name = getattr(self.text_encoder, "quantization", "none").upper()
            g.gui_text_encoder_mode = client.gui.add_dropdown(
                "Text Encoder",
                options=_text_encoder_options,
                initial_value=_text_encoder_initial,
                disabled=_quantized,
                hint=f"{_quantization_name} / BF16 量化编码器；切换设备需要重新加载。" if _quantized else "切换文本编码器设备和精度。",
            ...
            def _(event: viser.GuiEvent) -> None:
                ...
                try:
                    ...
                    with pause_for_model_reload(self.client_sessions[client_id]):
