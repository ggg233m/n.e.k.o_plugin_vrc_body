...
class EmbeddingCache:
    ...
    def _make_key(text: str) -> str:
        # 模型、精度清单、计算类型和编码路径已由外层目录指纹隔离；
        # 同一命名空间内再按原始提示词索引，不能跨精度复用向量。
    ...
    def get_or_encode(self, texts: list[str], encoder) -> tuple[torch.Tensor, list[int]]:
        ...
        if misses:
            ...
            miss_tensor = miss_tensor.detach()
            miss_arrays = miss_tensor.to(device="cpu", dtype=torch.float32).numpy()
            ...
            if len(misses) == len(texts):
                # 全部未命中时，持久化只需 CPU 副本，返回原设备张量避免再上传。
                return miss_tensor[:, :max(lengths)], lengths
...
class CachedTextEncoder:
    ...
    def __init__(
        ...
        self._refresh_cache_namespace()
        self._device, self._dtype = _probe_device_dtype(encoder)

    def _refresh_cache_namespace(self):
        # 旧缓存保留原目录；新本地模型按底座、适配器和精度配置隔离。
        fingerprint = getattr(self.encoder, "cache_fingerprint", None)
        directory = os.path.join(self._cache_root, fingerprint) if fingerprint else self._cache_root
        self.cache = EmbeddingCache(cache_dir=directory, max_mem_entries=self._max_mem_entries)
    ...
    def to(self, device=None, dtype=None):
        old_fingerprint = getattr(self.encoder, "cache_fingerprint", None)
        if hasattr(self.encoder, "to"):
            self.encoder.to(device=device, dtype=dtype)
        if old_fingerprint != getattr(self.encoder, "cache_fingerprint", None):
            self._refresh_cache_namespace()
