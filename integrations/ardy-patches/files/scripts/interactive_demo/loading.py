...
class ModelLoadingMixin:
    def _build_text_encoder(self):
        ...
        Uses TEXT_ENCODER_MODE (defaults to auto: remote service if reachable, otherwise in-
        ...
        return load_text_encoder(device=device)
    ...
    def load_model(self, client_id: int, model_name: str, progress=None):
        ...
        if torch.device(self.device).type == "xpu" and not getattr(session, "xpu_replan_defaults_applied", False):
            g = session.gui_elements
            if (g.gui_replan_trigger_thresh.value == DEFAULT_REPLAN_TRIGGER_THRESH
                    and g.gui_replan_buffer_size.value == DEFAULT_REPLAN_BUFFER_SIZE):
                # 按模型帧率预留生成时间，网页重连后也生效；保留用户已调整的值。
                trigger = max(1, min(model.gen_horizon_len - 4, round(1.8 * model_fps)))
                g.gui_replan_trigger_thresh.value = trigger
                g.gui_replan_buffer_size.value = max(0, trigger - 2)
            session.xpu_replan_defaults_applied = True
        ...
        if compile_mode != "None":
            ...
            try:
                if compile_mode.startswith("ONNX-TRT"):
                    ...
                    if getattr(self, '_use_core8_pipeline_startup', False) and model.gen_horizon_len == 8:
                        _progress("加载 Core8 专用编译路径；跳过原版重复预热。")
                    else:
