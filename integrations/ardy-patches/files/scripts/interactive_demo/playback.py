from .display_cache import display_frame
...
class PlaybackMixin:
    ...
    def set_frame(self, client_id: int, frame_idx: int, trigger_by_gui_timeline: bool = False):
        ...
        frame_data = display_frame(session, frame_idx) if session.joints_pos is not None and 0 <= frame_idx < session.joints_pos.shape[1] else None
        with session.characters_lock:
            if frame_data is not None:
                for character_idx, character in enumerate(session.characters.values()):
                    # 原版人物在 GPU 上蒙皮；VRM 在浏览器蒙皮，可直接使用 CPU 快照。
                    pose_data = frame_data if hasattr(character, 'vrm_handle') else frame_data['_source']
                    # Get actual root velocity for this frame (pass as tensor)
                    root_velocity = None
                    if session.root_velocities is not None:
                        root_velocity = pose_data['root_velocities'][character_idx]

                    foot_contacts = (
                        pose_data['foot_contacts'][character_idx] > 0.5
                        if session.foot_contacts is not None
                        else None
                    )
                    character.set_pose(
                        pose_data['joints_pos'][character_idx],
                        pose_data['joints_rot'][character_idx],
